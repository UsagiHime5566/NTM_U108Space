Shader "Unlit/NewUnlitShader 1"
{
    Properties
    {
        ...
        _SandScrollSpeed("_SandSrollSpeed" , float) = 1


        _MainColor("_MainColor " , color) = (1,1,1,1)
        _MainLightColor("_MainLightColor " , color) = (1,1,1,1)
        _LightDir("_LightDir " , Vector) = (1,1,1,1)
        _ViewDir("_ViewDir " , Vector) = (1,1,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 GetSurfaceNormal(float2 uv, float3 temNormal)
            {
                ...
                float3 shallowZ = UnpackNormal(tex2D(_NormalMapShallowZ, _NormalMapShallowZ_ST.xy * uv.xy + _NormalMapShallowZ_ST.zw + (_SandScrollSpeed * _Time.y)));
                float3 shallow = shallowX * shallowZ * _ShallowBumpScale;


                float3 steepX = UnpackNormal(tex2D(_NormalMapSteepX, _NormalMapSteepX_ST.xy * uv.xy + _NormalMapSteepX_ST.zw + (_SandScrollSpeed * _Time.y)));
                float3 steepZ = UnpackNormal(tex2D(_NormalMapSteepZ, _NormalMapSteepZ_ST.xy * uv.xy + _NormalMapSteepZ_ST.zw + (_SandScrollSpeed * _Time.y)));
